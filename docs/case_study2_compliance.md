...
| Low Latency | `scripts/load_test.py`, `reports/latency_judge_demo.md`, `reports/latency_fast_route.md` | Faster local wording mode kept all three routes at about `100 ms p95` or below in the current mixed exact-case local run |
...
- low-literacy mode changes the wording itself and intentionally uses the simplest deterministic local wording
